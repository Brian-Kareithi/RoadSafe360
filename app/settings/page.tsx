...
import { FiSave, FiSliders, FiSettings, FiShield } from 'react-icons/fi';
    ...
    <div className="space-y-6 max-w-3xl animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[var(--text-muted)] text-sm mt-1">Configure system thresholds and preferences</p>
        </div>
          ...
          {demeritSettings.map((s: any) => (
            <div key={s.id} className="flex items-center justify-between gap-4 py-1">
              <div className="flex-1">
                <label className="text-sm font-semibold text-[var(--text)]">{s.description || s.key}</label>
                <p className="text-xs text-[var(--text-light)] font-mono mt-0.5">{s.key}</p>
              </div>
              <Input value={formValues[s.key] || ''} onChange={e => setFormValues({ ...formValues, [s.key]: e.target.value })} className="w-32 text-right font-mono" />
            </div>
          ))}
          {demeritSettings.length === 0 && <p className="text-sm text-[var(--text-muted)] py-6 text-center">No threshold settings found in database</p>}
        </CardContent>
      </Card>

      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2"><FiSettings size={16} /> General Settings</CardTitle>
        ...
        <CardContent className="space-y-5">
          ...
          {generalSettings.length === 0 && <p className="text-sm text-[var(--text-muted)] py-6 text-center">No general settings found in database</p>}
        </CardContent>
      </Card>
    </div>
  );
