...
export default function AuthorityDashboard() {
  ...
  return (
    <div className="space-y-6 py-6">
      <div>
        <h1 className="text-2xl font-bold">Road Authority Dashboard</h1>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              ...
              <FiUsers className="h-8 w-8 text-blue-500 opacity-50" />
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              ...
              <FiShield className="h-8 w-8 text-red-500 opacity-50" />
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              ...
              <FiTrendingUp className="h-8 w-8 text-green-500 opacity-50" />
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              ...
              <FiDollarSign className="h-8 w-8 text-emerald-500 opacity-50" />
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {[
              ...
              <div key={kpi.label} className="flex justify-between items-center py-1">
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent>
            <div className="space-y-3">
              {drivers.filter((d: any) => (d.riskScore || 0) >= 0.4).slice(0, 5).map((d: any) => (
                <div key={d.id} className="flex items-center justify-between py-2 border-b border-zinc-100 dark:border-zinc-800 last:border-0">
                  <div>
                    <p className="text-sm font-medium">{d.fullName}</p>
                    <p className="text-xs text-zinc-500">{d.nationalID}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-24 h-2 rounded-full bg-zinc-200 dark:bg-zinc-700">
                      <div className="h-full rounded-full bg-red-500" style={{ width: `${Math.min((d.riskScore || 0) * 100, 100)}%` }} />
                    </div>
                    <span className="text-sm font-medium">{((d.riskScore || 0) * 100).toFixed(0)}%</span>
                  </div>
                </div>
              ))}
              {drivers.filter((d: any) => (d.riskScore || 0) >= 0.4).length === 0 && <p className="text-sm text-zinc-400">No high-risk drivers identified</p>}
            </div>
          </CardContent>
        </Card>
      </div>
